...
It required changes to the `vercel.json` file that can be read up on [here](https://developers.google.com/tag-platform/security/guides/csp#google_analytics_4_google_analytics).
...
and optionally updating the `<script>` tag in the Vue.js project to include the hash:

```html
<script integrity="sha256-NWf2QfXgstC58zeipPN/8CH5ZLaYLMFh0+dDWU0xbcY=" crossorigin="anonymous">
```

This is a [quick read](https://content-security-policy.com/hash/) on hashes with CSP that I found helped.
