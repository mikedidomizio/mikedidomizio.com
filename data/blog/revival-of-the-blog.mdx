---
title: Deploying a Next.js app with a Vue.js app on Vercel (Revival of the Blog)
date: '2024-01-29'
...
I'm hoping uou can read how I did it and apply what I learned to your own situation.

If you want to jump straight to how it was done, [click here](#how-to).
...
In 2024, I decided to shut it down because it was just unnecessary hosting costs and I wasn't using it much.
...
even checked that it was under two pages when printed.
...
but I haven't really worked on PHP in many years. I chose to go the JavaScript route as it's something I've
...
## Setting up multiple apps in Vercel under one domain

The rest of this is going to be discussing how you can have multiple projects under one domain deploying with Vercel.
...
Two separate git histories, easy to maintain and build separately and if I need to separate them again, it's fairly easy to do so.

### Changing the Vercel project configuration and deploying with Vercel

[Vercel](https://vercel.com) is a PaaS company that maintains the Next.js project. They allow you to
use their platform to deploy, monitor, and maintain projects (not just Next.js) with a lot of ease things
that you would once have to manually be setup all by yourself.
...
I was immediately hit with one issue though which is the CSS/JS not loading properly in the résumé.
The simple solution would be to also forward those routes as well, but I chose to have Vue build to differentiate
these files as `resume-*` directories/files because `css` and `js` directories are pretty generic and I didn't want any conflicts.
...
Google Analytics (referred here on as GA) was also blocked by CSP. The main site had not been setup and the résumé GA
