...
As the title indicates, this website is a primarily Next.js, with Vue.js bolted on for some endpoints, and deployed via Vercel.
Using Next.js to power the blog, and Vue.js for the CV/Resume.

You can read how I did it and apply what I learned to your own situation.

If you want to jump straight to how it was done, [click here](#the-issues).

## Intro

Just to give a bit of history, the original blog was a WordPress installation.
...
It's something I suggest everyone to do.
...
I wanted to have easy to maintain consistency throughout the page. Which could be done by breaking the CV into components.
...
My original thought was to go back to WordPress,
but I haven't really worked on PHP 5.X. I chose to go the JavaScript route as it's something I've
become more familiar with over time and work with every day.

I thought to go with Remix but I'm still not sure I'm a fan of it.
I use Next.js with a lot more of my projects and am more comfortable with it.
...
Two separate git histories, easy to maintain separately and if I need to separate them again, it's fairly easy to do so.

## Creating the vercel.json configuration

Vercel has the ability to include a `vercel.json` file to alter the [project configuration](https://vercel.com/docs/projects/project-configuration).
...
This was accomplished pretty simply with just having the `source` be the endpoint I wanted, and the `destination` point to the other Vercel deployment in my `vercel.json` file:
...
I was immediately hit with one issue though which is the CSS/JS not loading properly.
...
The Vercel configuration change is mandatory, but would differ if you take a different approach.
...
Google Analytics (referred sometimes here as GA) was also blocked by CSP. The main site had not been setup and the Resume/CV Google Analytics
was no longer working properly.

The Resume/CV deployment required changes to the `vercel.json` file,
...
I like that approach. You can verify the above works by loading up Google Analytics Real Time dashboard, visiting the endpoint in another tab and see if GA reports a new user.
...
This should be fairly stable with Google Analytics 4, but it's always possible things change.
...
Have you done a similar or alternate setup? I'm curious to see what you have done.
<TweetIntent text="Let me know!" tweet="@Mike_DiDomizio re:deploying multi apps under Vercel." />.
